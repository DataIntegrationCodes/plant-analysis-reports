...
const PAR = {
  ...
  // closes with the wind resource deviation. Downtime-cause attribution
  // (e.g. "dominated by grid curtailments") is deliberately not used here -
  // that breakdown isn't reliable/available yet per the model owner, even
  // though the raw per-category numbers exist in the data.
  buildProjectSummary(plant) {
    ...
    let lossSentence = "";
    if (w.deviation !== null && w.deviation !== undefined) {
